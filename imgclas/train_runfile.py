...
from imgclas.data_utils import (
    load_data_splits,
    compute_meanRGB,
...
def train_fn(TIMESTAMP, CONF):
    ...
    assert CONF["model"]["num_classes"] >= np.amax(
        y_train
    ), "Your train.txt file has more categories than those defined in classes.txt"  # nosec
    if CONF["training"]["use_validation"]:
        assert CONF["model"]["num_classes"] >= np.amax(
            y_val
        ), "Your val.txt file has more categories than those defined in classes.txt"  # nosec
